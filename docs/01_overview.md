...
This repository implements an end-to-end network security intelligence system centered on the `nal` application. It is designed to analyze network traffic flows, classify known attack types, detect anomalous behavior, assign risk levels, and expose results through APIs and dashboards.
...
- Runtime and model integrity endpoints for operational safety checks.
...
- Integrate backend outputs with an external alerting system of your choice.
...
6. Frontend and external automation systems consume API outputs.
...
- **Fail-fast behavior:** if required model artifacts are missing, inference returns explicit `MODEL_UNAVAILABLE` state.
