...
def process_directory(input_dir, output_dir, fields=None, verbose=False):
    ...
    # Find all pcap files (including extensionless pcap/pcapng)
    pcap_files = list(input_path.glob("*.pcap")) + list(input_path.glob("*.pcapng"))
    for f in input_path.iterdir():
        if f.is_file() and f.suffix == "" and f not in pcap_files:
            try:
                with open(f, "rb") as _fh:
                    if _fh.read(4) in (b"\xd4\xc3\xb2\xa1", b"\x0a\x0d\x0d\x0a"):
                        pcap_files.append(f)
            except Exception:
                pass
